"""Extract ordinary-form BSL module text from 1C dump `Form.bin` containers.

See `Модуль-формы-из-Form.bin.md`. Typical dump path:
`…/Forms/<Form>/Ext/Form.bin` (UTF-8 module blob + form serialization).
"""
...
def form_bin_to_module_path(path: str) -> str:
    """Map `…/Ext/Form.bin` → `…/Ext/Form/Module.bsl` for resolve_bsl_module."""
...
def diagnose_form_bin(data: bytes) -> str:
    ...
    # printable head preview (escaped)
...
def _trim_module_text(raw: str) -> str | None:
    ...
    # Trim address table / form serialization after last procedure end
    tail_from = ends[-1]
...
def extract_form_module_bsl(data: bytes) -> str:
    ...
    if data.find(_PROC) < 0 and data.find(_FUNC) < 0:
        # cp1251 (or other) keywords present — real extract problem, not "empty"
        raise FormBinError(f"no UTF-8 BSL (Процедура) in Form.bin | {diagnose_form_bin(data)}")
    ...
    def flush() -> None:
        ...
        try:
            ...
            if ("Процедура" not in s) and ("Функция" not in s) and ("КонецПроцедуры" not in s):
                buf = bytearray()
                return
